...
namespace {
FLAC__StreamDecoderReadStatus streamRead(const FLAC__StreamDecoder *,
                                         FLAC__byte buffer[],
                                         std::size_t *bytes, void *clientData) {
...
FLAC__StreamDecoderSeekStatus streamSeek(const FLAC__StreamDecoder *,
...
FLAC__StreamDecoderTellStatus streamTell(const FLAC__StreamDecoder *,
                                         FLAC__uint64 *absoluteByteOffset,
                                         void *clientData) {
  sf::priv::SoundFileReaderFlac::ClientData *data =
      static_cast<sf::priv::SoundFileReaderFlac::ClientData *>(clientData);
...
FLAC__StreamDecoderLengthStatus streamLength(const FLAC__StreamDecoder *,
...
FLAC__StreamDecoderWriteStatus streamWrite(const FLAC__StreamDecoder *,
  ...
  if (data->remaining < frameSamples)
    data->leftovers.reserve(
        static_cast<std::size_t>(frameSamples - data->remaining));
    ...
    for (unsigned int j = 0; j < frame->header.channels; ++j) {
      ...
      if (data->buffer && data->remaining > 0) {
        ...
        *data->buffer++ = sample;
        ...
        // We are either seeking (null buffer) or have decoded all the requested
        // samples during a normal read (0 remaining), so we put the sample in a
        // temporary buffer until next call
...
void streamMetadata(const FLAC__StreamDecoder *,
                    const FLAC__StreamMetadata *meta, void *clientData) {
  sf::priv::SoundFileReaderFlac::ClientData *data =
      static_cast<sf::priv::SoundFileReaderFlac::ClientData *>(clientData);
...
void streamError(const FLAC__StreamDecoder *, FLAC__StreamDecoderErrorStatus,
                 void *clientData) {
  sf::priv::SoundFileReaderFlac::ClientData *data =
      static_cast<sf::priv::SoundFileReaderFlac::ClientData *>(clientData);
  data->error = true;
...
bool SoundFileReaderFlac::check(InputStream &stream) {
  ...
  data.stream = &stream;
  data.error = false;
  FLAC__stream_decoder_init_stream(decoder, &streamRead, &streamSeek,
                                   &streamTell, &streamLength, &streamEof,
...
////////////////////////////////////////////////////////////
SoundFileReaderFlac::SoundFileReaderFlac() : m_decoder(NULL), m_clientData() {}

////////////////////////////////////////////////////////////
SoundFileReaderFlac::~SoundFileReaderFlac() { close(); }
...
bool SoundFileReaderFlac::open(InputStream &stream, Info &info) {
  ...
  if (!m_decoder) {
    err() << "Failed to open FLAC file (failed to allocate the decoder)"
          << std::endl;
    return false;
  }
  ...
  FLAC__stream_decoder_init_stream(
      m_decoder, &streamRead, &streamSeek, &streamTell, &streamLength,
      &streamEof, &streamWrite, &streamMetadata, &streamError, &m_clientData);
...
void SoundFileReaderFlac::seek(Uint64 sampleOffset) {
  ...
  // FLAC decoder expects absolute sample offset, so we take the channel count
  // out
  if (sampleOffset < m_clientData.info.sampleCount) {
    // The "write" callback will populate the leftovers buffer with the first
    // batch of samples from the seek destination, and since we want that data
    // in this typical case, we don't re-clear it afterward
    FLAC__stream_decoder_seek_absolute(
        m_decoder, sampleOffset / m_clientData.info.channelCount);
  } else {
    // FLAC decoder can't skip straight to EOF, so we short-seek by one sample
    // and skip the rest
    FLAC__stream_decoder_seek_absolute(
        m_decoder,
    ...
    // This was re-populated during the seek, but we're skipping everything in
    // this, so we need it emptied
    m_clientData.leftovers.clear();
  }
}
...
Uint64 SoundFileReaderFlac::read(Int16 *samples, Uint64 maxCount) {
  ...
  if (left > 0) {
    if (left > maxCount) {
      ...
      std::copy(m_clientData.leftovers.begin(), m_clientData.leftovers.end(),
                samples);
    }
  }
  ...
  // Decode frames one by one until we reach the requested sample count, the end
  // of file or an error
