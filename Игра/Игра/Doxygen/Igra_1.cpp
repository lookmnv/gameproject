...
using namespace std;
using namespace sf;
/**The function responsible for the order of the columns
\author Saida
*/
int stolb[3];
/**Function where the field is reset and blocks are placed
\author Saida
\param field playing field
*/
int nullpole(int field[]) {
    for (int i = 0; i < 25; i++) {
        field[i] = 0;
    }
...
/** Function that checks whether the columns match the upper columns
    if they match, it returns 1; otherwise, it returns 0
    \author Saida
    \param field playing field
    \param orderCubesToBeEnd order of cubes
*/
int proverka(int field[], int orderCubesToBeEnd[]) {
...
/**Function responsible for the placement of cubes on the field. Random placement is performed
\author Saida
\param field playing field
*/
int arrangement(int field[]) {
...
/**A function that sets the random order of columns to be collected
\author Angelina
\param order array to hold result of generating order of columns
*/
int stolbi(int order[]) {
    srand(time(0));
...
/**Function that outputs the current state of the field
\author Angelina
\param field playing field
*/
int matrixShow(int field[]) {
    for (int i = 0; i < 25; i++) {
        if (i % 5 == 0) {
            cout << endl;
        }
...
/**This is where the correct movement of blocks is checked, so we don't want to go from cell 4 to cell 15,
if this can be done, we replace the values in the matrix and redraw the field
\author Angelina
\param field playing field
\param vibor1 choice 1 of player
\param vibor2 choice 2 of player
*/
int ModelBox(int field[], int vibor1, int vibor2) {
    int temp, complete = 0;
    complete = 0;
...
/**Displays the order of columns
\author Angelina
\param orderCubesToBeEnd order of cubes
*/
void vivodporyadka(int orderCubesToBeEnd[]) {
    for (int i = 0; i < 3; i++)
        cout << orderCubesToBeEnd[i] << "   ";
}

/**If a box is selected and the last click was not on it, then the box will no longer be selected
\author Saida
\param matrica array of field with one selected
\param poslvidel index of selected cell
*/
void selectCheck(int matrica[], int poslvidel) {
    for (int i = 0; i < 25; i++) {
...
/**Here we find out where we hit the cursor
\author Saida
\param pozx x index of cursor
\param pozy y index of cursor
\param x horizontal index of cursor
\param y vertical index of cursor
*/
void determineHit(int* x, int* y, int pozx, int pozy) {
...
int main() {
    /**Creating a window and playing field
    \author Saida
        */
    ...
    do {
       /**Eventhandler, which manages callbacks to be executed.
        \author Angelina
            */
        Event event;
        win = false;
        ...
        /**Randomly arrange the cubes on the field
        \author Angelina
            */
        arrangement(field);

        /**Output the field matrix
        \author Angelina
            */
        matrixShow(field);
        while (window.isOpen() && win == false) {
            ...
            while (window.pollEvent(event)) {
                        ...
                        while (fclick == false) {
                            ...
                            for (int i = 0; i < 25; i++) {
                                ...
                                if ((pos.x > 100 + 84 * modi) && (pos.x < 100 + 84 * modi + 64) && (pos.y > 100 + 84 * divi) && (pos.y < 100 + 84 * divi + 64)) {
                                    ...
                                    /**If we click on a blocked block or an empty one, the previous block will be deselected
                                    \author Angelina
                                        */
                                    ...
                                    /**If the number of selected cubes is more than 1, we will make the previous cube not selected
                                    \author Angelina
                                        */
                                    if (countSelected > 1) {
                ...
                /**If we haven't selected anything yet, the cube we clicked will be selected
                \author Angelina
                    */
                if (field[firstSelect] != 0) {
                    if (countSelected == 1)
                        ...
                        if (firstSelect != 1 || firstSelect != 3 || firstSelect != 11 || firstSelect != 13 || firstSelect != 21 || firstSelect != 23) {
                            ...
                            for (int i = 0; i < 25; i++) {
                                ...
                                if ((pos.x > 100 + 84 * modi) && (pos.x < 100 + 84 * modi + 64) && (pos.y > 100 + 84 * divi) && (pos.y < 100 + 84 * divi + 64)) {
                                    ...
                                    /**If the cursor is not on the blocked block and the second click is not equal to the first click,
                                    then we move the box from the first click to the free field
                                        \author Angelina */
            ...
            for (int i = 0; i < 25; i++) {
                x = i % 5;
                y = i / 5;
                /**Drawing boxes that are located above the playing field
                \author Angelina
                    */
            ...
            if (proverka(field, stolb) == 1) {/**If all the columns are collected in the correct order, then we go into this cycle
                Draw the background, because since the game has stopped, you need to draw the last frame
                \author Saida
                */
                window.clear();
                window.draw(fonspr);
                ...
                /**Wait until the restart button is clicked, and then restart the game
                \author Saida
                    */
                while (restclick == false) {
                    Vector2i pixelPos = Mouse::getPosition(window);
                    Vector2f pos = window.mapPixelToCoords(pixelPos);
                    system("cls");
                    Event event1;
                    while (window.pollEvent(event1)) {
                        ...
                        if (event1.type == Event::MouseButtonPressed)
                            if (event1.key.code == Mouse::Left)
                                ...
                                /**If you hit the button sprite, the game will restart
                                \author Saida
                                */
